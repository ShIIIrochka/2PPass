...
def get_side_menu(
	...
	for app in available_apps:
		...
		custom_link_names = [
			x.get("name", "").lower() for x in app_custom_links
		]
		model_ordering = list(
			filter(
	...
	for choice in choices:
		...
		for key in query_parts.keys():
			if key == field_key:
				...
				key.startswith(field_key + "__")
				or "__" + field_key + "__" in key
			):
				value = query_parts[key][0]
				matched_key = key
...
def get_changeform_template(adminform: AdminForm) -> str:
	...
	if (
		not changeform_format
		or changeform_format not in CHANGEFORM_TEMPLATES.keys()
	):
...
def action_message_to_list(action: LogEntry) -> List[Dict]:  # noqa: C901
	...
	return (
		messages if len(messages) else [changed(gettext(action.change_message))]
	)
